class Batter < ActiveRecord::Base
  ...
  def batterTotalstats(id)

    s = Batter.where(player_id: id)
  ...
  def updateStats(player)
    ...
    items = doc.xpath("//tr[starts-with(@id,'batting_standard.2016')]").collect {|node| node.text.strip}

    items.each{|i|
      
      t = i.split("\n")
      stats = Batter.where(year: "2016").where(player_id: player.id)[0]
